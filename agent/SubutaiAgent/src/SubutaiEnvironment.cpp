...
/**
 *  \details   SubutaiAgent's settings.xml is read by this function.
...
int SubutaiEnvironment::getAgentSettings() {
	...
	if (doc.load_file("/etc/subutai-agent/agent.xml").status) //if the settings file does not exist
	{
		environmentLogger->writeLog(7,
				environmentLogger->setLogData(
						"<SubutaiEnvironment::getAgentSettings>",
						"Agent.xml cannot be read!"));
		environmentLogger->writeLog(7,
				environmentLogger->setLogData(
						"<SubutaiEnvironment::getAgentSettings>",
						"Agent is closing now."));
	...
	environmentLogger->writeLog(6,
			environmentLogger->setLogData("<SubutaiAgent>", "ConnectionUrl: ",
					connectionUrl));
	environmentLogger->writeLog(6,
			environmentLogger->setLogData("<SubutaiAgent>", "ConnectionPort: ",
					connectionPort));
	environmentLogger->writeLog(6,
			environmentLogger->setLogData("<SubutaiAgent>",
					"ConnectionOptions:", connectionOptions));
	environmentLogger->writeLog(6,
			environmentLogger->setLogData("<SubutaiAgent>", "LogLevel:",
					logLevel));
	environmentLogger->writeLog(6,
			environmentLogger->setLogData("<SubutaiAgent>",
					"Agent.xml is read successfully.."));
	int loglevel;
...
bool SubutaiEnvironment::getAgentUuid() {
	try {
		...
		if (this->uuid.empty())	//if uuid is null or not reading successfully
		{
			...
			file << this->uuid;
			file.close();
			environmentLogger->writeLog(1,
					environmentLogger->setLogData("<SubutaiAgent>",
							"Subutai Agent UUID: ", this->uuid));
			return false;
		}
		return true;
...
string SubutaiEnvironment::getAgentArch() {
	utsname info;
	if (uname(&info) != 0) {
		environmentLogger->writeLog(3,
				environmentLogger->setLogData("<SubutaiAgent>",
						"Error: Failed to extract machine information"));
		return "UNKNOWN";
	} else {
