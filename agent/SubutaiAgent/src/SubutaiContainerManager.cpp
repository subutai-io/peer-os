...
SubutaiContainerManager::SubutaiContainerManager(string lxc_path,
	...
	try {
		...
		_logger->writeLog(3,
				_logger->setLogData("<SubutaiContainerManager>", e.what()));
	}
}
...
void SubutaiContainerManager::updateContainerIdListOnStart() {
	...
	while (file >> hostname_file >> id) {
		for (ContainerIterator it = _containers.begin();
			...
			if (!strcmp(hostname_file.c_str(),
					(*it)->getContainerHostnameValue().c_str())) {
	...
	if (fp) {
		...
		if ((getline(&line, &n, fp) > 0)) {
			...
			if (strcmp(line, "CONTAINER") != 0) {
				_logger->writeLog(7,
						_logger->setLogData("<SubutaiContainerManager>",
								"Return value of 'subutai list -c' is changed.. The first line is not 'CONTAINER'. It may cause problems, please check."));
			}
		}
		if ((getline(&line, &n, fp) > 0)) {
			get_word_from_line(line, static_cast<int>(n));
			if (strcmp(line, "---------") != 0) {
				_logger->writeLog(7,
						_logger->setLogData("<SubutaiContainerManager>",
								"Return value of 'subutai list -c' is changed.. The second line is not '---------'. It may cause problems, please check."));
			}
		}
 ...
 *
 */
bool SubutaiContainerManager::checkIfTemplate(string container_name,
		vector<string> containerList, bool isSubutaiAvailable) {
	// Template - container difference is only available for Subutai systems, otherwise always return true.
	if (!isSubutaiAvailable)
		return false;
	for (vector<string>::const_iterator it = containerList.begin();
 ...
 *
 */
SubutaiContainer* SubutaiContainerManager::findContainerById(
		string container_id) {
...
void SubutaiContainerManager::updateContainerLists() {
	...
	try {
		...
		/*
		 * At this step if there is an execution of lxc destroy command,
		...
		for (int i = 0; i < num; i++) {
			if (!checkIfTemplate(names[i], subutai_containers,
				...
				if (!containerFound) {
					SubutaiContainer* c = new SubutaiContainer(_logger,
							cont[i]);
					c->setContainerHostname(names[i]);
