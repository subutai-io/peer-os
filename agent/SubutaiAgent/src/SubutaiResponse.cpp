 ...
 */
void SubutaiResponse::clear() {		//clear the all variables..
	setType("");
	setUuid("");
	setPid(-1);
	setRequestSequenceNumber(-1);
...
void SubutaiResponse::serialize(string& output) {
	...
	for (unsigned int index = 0; index < this->getInterfaces().size();
			index++) {
		root["response"]["interfaces"][index]["interfaceName"] =
				this->getInterfaces()[index].name;
		root["response"]["interfaces"][index]["ip"] =
				this->getInterfaces()[index].ip;
		root["response"]["interfaces"][index]["mac"] =
				this->getInterfaces()[index].mac;
	}
	...
	if (this->getType() != "INOTIFY_EVENT") {
		for (unsigned int index = 0; index < this->containers.size(); index++) {
			root["response"]["containers"][index]["hostname"] =
					this->containers[index]->getContainerHostnameValue();
			root["response"]["containers"][index]["id"] =
					this->containers[index]->getContainerIdValue();
			root["response"]["containers"][index]["status"] =
					this->containers[index]->getContainerStatus();
			if (this->containers[index]->getContainerArch() != "") {
				root["response"]["containers"][index]["arch"] =
						this->containers[index]->getContainerArch();
			}
			vector<Interface> interfaceValues =
					this->containers[index]->getContainerInterfaceValues();
			for (unsigned int i = 0; i < interfaceValues.size(); i++) {
				root["response"]["containers"][index]["interfaces"][i]["interfaceName"] =
						interfaceValues[i].name;
				root["response"]["containers"][index]["interfaces"][i]["ip"] =
						interfaceValues[i].ip;
				root["response"]["containers"][index]["interfaces"][i]["mac"] =
						interfaceValues[i].mac;
			}
		}
	}
	for (unsigned int index = 0; index < this->getConfPoints().size();
			index++) {
		if (this->getType() == "LIST_INOTIFY_RESPONSE") {
			root["response"]["configPoints"][index] =
					this->getConfPoints()[index];
		} else if (this->getType() == "INOTIFY_EVENT") {
			root["response"]["configPoint"][index] =
					this->getConfPoints()[index];
		}
	}
 ...
 *  		   It returns given reference output strings.
 */
void SubutaiResponse::serializeDone(string& output) {//Serialize a Done Response  to a Json String
#if USE_PROTOBUF
		Subutai::Response response;
		if (!this->getType().empty()) {
			if (this->getType() == "HEARTBEAT") {
 ...
 */
int SubutaiResponse::getPid() {						//getting pid
	return this->pid;
}
 ...
 */
void SubutaiResponse::setPid(int pid) {			//setting pid
	this->pid = pid;
}
 ...
 */
int SubutaiResponse::getExitCode() {					//getting ExitCode
 ...
 */
void SubutaiResponse::setExitCode(int exitcode) {			//setting ExitCode
 ...
 */
string& SubutaiResponse::getType() {							//getting Type
	return this->type;
}
 ...
 */
void SubutaiResponse::setType(const string& type) {				//setting Type
	this->type = type;
}
 ...
 */
string& SubutaiResponse::getUuid() {							//getting uuid
	return this->uuid;
}
 ...
 */
void SubutaiResponse::setUuid(const string& uuid) {				//setting uuid
	this->uuid = uuid;
}
 ...
 */
int SubutaiResponse::getRequestSequenceNumber() {	//getting RequestSeqnumber
 ...
 */
void SubutaiResponse::setRequestSequenceNumber(int requestSequenceNumber) {	//setting RequestSeqnumber
 ...
 */
int SubutaiResponse::getResponseSequenceNumber() {	//getting ResponseSeqnumber
 ...
 */
void SubutaiResponse::setResponseSequenceNumber(int responseSequenceNumber) {//setting ResponseSeqnumber
 ...
 */
string& SubutaiResponse::getStandardError() {			//getting standard err
	return this->stdErr;
}
/**
 *  \details   setting "stdErr" private variable of SubutaiResponse instance
 */
void SubutaiResponse::setStandardError(const string& mystderr) {//setting standard err
 ...
 */
string& SubutaiResponse::getStandardOutput() {			//getting standard out
	return this->stdOut;
}
 ...
 */
void SubutaiResponse::setStandardOutput(const string& mystdout) { //setting standard out
 ...
 */
vector<string>& SubutaiResponse::getConfPoints() {			//getting ips vector
