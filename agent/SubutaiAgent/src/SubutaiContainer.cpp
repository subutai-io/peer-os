...
SubutaiContainer::~SubutaiContainer() {
	...
	containerLogger->writeLog(6,
			containerLogger->setLogData(_logEntry,
					"Destroying " + this->getContainerHostnameValue()));
}
 ...
 *
 */
ExecutionResult SubutaiContainer::RunProgram(string program,
		vector<string> params, bool return_result, lxc_attach_options_t opts,
		bool captureOutput) {
	if (this->getState() != "RUNNING")
		throw new SubutaiException(
				"Execution command on a not running container", 101);
	...
	try {
		result.exit_code = this->container->attach_run_wait(this->container,
				&opts, _params[0], _params);
	} catch (std::exception e) {
		containerLogger->writeLog(3,
				containerLogger->setLogData(_logEntry,
						"Execution failed (LXC): " + string(e.what())));
	}
	if (captureOutput) {
		char outbuf[4000];
	...
	/*
	 containerLogger->writeLog(7, containerLogger->setLogData(
	 _logEntry,
...
void SubutaiContainer::UpdateUsersList() {
	...
	string passwd = RunProgram("/bin/cat", params, true,
			LXC_ATTACH_OPTIONS_DEFAULT, true).out;
...
bool SubutaiContainer::getContainerId() {
	try {
		...
		if (hostname.empty())	//if uuid is null or not reading successfully
		{
			...
			/*
			 containerLogger->writeLog(7,containerLogger->setLogData(_logEntry,"Subutai container ID: ",
			 this->id, " is generated for " + this->hostname + " and added to file."));
			 */
			return false;
		}
		/*
		 containerLogger->writeLog(7,containerLogger->setLogData(_logEntry,"Found container ID: ",
		 this->id + " for " + this->hostname));*/
...
bool SubutaiContainer::getContainerInterfaces() {
	interfaces.clear();
	//containerLogger->writeLog(7, containerLogger->setLogData(_logEntry, "Updating interface information"));
...
string SubutaiContainer::getContainerArch() {
	if (_arch == "" && getState() == "RUNNING") {
		try {
			...
			std::transform(_arch.begin(), _arch.end(), _arch.begin(),
					::toupper);
		} catch (std::exception e) {
			containerLogger->writeLog(3,
					containerLogger->setLogData(_logEntry,
...
ExecutionResult SubutaiContainer::RunCommand(SubutaiCommand* command) {
	containerLogger->writeLog(6,
			containerLogger->setLogData(_logEntry, "Running command.. "));
...
string	> >::iterator it;
	int i = 0;
	for (it = command->getEnvironment().begin();
			it != command->getEnvironment().end(); it++, i++) {
		stringstream ss;
	...
	if (hasSubCommand(command)) {
		...
		for (vector<string>::iterator it = pr.begin() + 1; it != pr.end();
				it++) {
			args.push_back((*it));
		}
	}
 ...
 *
 */
ExecutionResult SubutaiContainer::RunCommandAsDaemon(SubutaiCommand* command,
		int* sub_pid) {
	SubutaiHelper h;
	ExecutionResult res = RunDaemon(command);
	*sub_pid = res.pid;
	//containerLogger->writeLog(7, containerLogger->setLogData(_logEntry,
	//		"Running non-daemon command " + command->getCommand()));
	return res;
}
...
ExecutionResult SubutaiContainer::RunDaemon(SubutaiCommand* command) {
	if (getState() != "RUNNING")
		throw new SubutaiException(
				"Trying to run daemon on a non running container", 100);
	...
	if (command->getWorkingDirectory() != ""
			&& checkCWD(command->getWorkingDirectory())) {
		//containerLogger->writeLog(7, containerLogger->setLogData(_logEntry, "change working directory.."));
		opts.initial_cwd =
				const_cast<char*>(command->getWorkingDirectory().c_str());
	}
	...
	if (hasSubCommand(command) || exceptional_command) {
		...
		char* args[pr.size() + 1];
		int i = 0;
		for (vector<string>::iterator it = pr.begin(); it != pr.end();
				it++, i++) {
			if (it == pr.begin()) {
...
bool SubutaiContainer::checkCWD(string cwd) {
	...
	ExecutionResult result = RunProgram("ls", params, false,
			LXC_ATTACH_OPTIONS_DEFAULT);
	if (result.exit_code == 0) {
		return true;
	} else {
		containerLogger->writeLog(3,
				containerLogger->setLogData(_logEntry,
						"working directory not found: " + cwd + "on "
								+ this->hostname));
		return false;
	}
}
...
bool SubutaiContainer::checkUser(string username) {
	...
	containerLogger->writeLog(3,
			containerLogger->setLogData(_logEntry,
					"User not found: " + username + "on " + this->hostname));
	return false;
}
 ...
 */
vector<string> SubutaiContainer::ExplodeCommandArguments(
...
string SubutaiContainer::getState() {
	try {
		...
		containerLogger->writeLog(3,
				containerLogger->setLogData(_logEntry, string(e.what())));
	}
}
