...
bool SubutaiContainer::getContainerId()
{
    try
    {
        ...
        {
            containerLogger->writeLog(1, containerLogger->setLogData(_logEntry, this->hostname, " cannot be found in containerId.txt. Generate uuid.."));
            boost::uuids::random_generator gen;
            boost::uuids::uuid u = gen();
            const std::string tmp = boost::lexical_cast<std::string>(u);
            this->id = tmp;

            containerLogger->writeLog(1, containerLogger->setLogData(_logEntry, "write generated uuid into containerId.txt.."));
            ...
            containerLogger->writeLog(1,containerLogger->setLogData(_logEntry,"Subutai container ID: ",this->id));
            return false;
        }
        containerLogger->writeLog(1,containerLogger->setLogData(_logEntry,"Found container ID: ",this->id));
...
ExecutionResult SubutaiContainer::RunDaemon(SubutaiCommand* command) {
    ...
    pid_t pid;
    lxc_attach_options_t opts = LXC_ATTACH_OPTIONS_DEFAULT;
    containerLogger->writeLog(1, containerLogger->setLogData(_logEntry, "Running daemon. "));
    ...
    containerLogger->writeLog(1, containerLogger->setLogData(_logEntry, "Daemon pid: ", h.toString(pid)));
    containerLogger->writeLog(1, containerLogger->setLogData(_logEntry, "Exit code: ", h.toString(ret)));
