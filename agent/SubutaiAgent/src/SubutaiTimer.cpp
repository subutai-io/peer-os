 ...
 */
SubutaiTimer::SubutaiTimer(SubutaiLogger log, SubutaiEnvironment* env,
		SubutaiContainerManager* cont, SubutaiConnection* conn) {
 ...
 */
bool SubutaiTimer::checkExecutionTimeout(unsigned int* startsec, bool* overflag,
		unsigned int* exectimeout, unsigned int* count) {
	if (*exectimeout != 0) {
		...
		*heartbeatIntFlag = false;
		...
		logMain.writeLog(7,
				logMain.setLogData("<SubutaiTimer>",
						"Lxc destruction command is in progress, ",
		...
		if (queueFile.peek() != ifstream::traits_type::eof()) {
			while (getline(queueFile, queueElement)) {
				if (command->deserialize(queueElement)) {
					...
					logMain.writeLog(7,
							logMain.setLogData("<SubutaiTimer>",
									"Cannot deserialize: ", queueElement));
				}
			}
		}
		queueFile.close();
