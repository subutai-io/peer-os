...
public class StartNodeOperationHandler extends AbstractOperationHandler<MongoImpl>
{
    ...
    public void run()
    {
        ...
        {
            startNodeCommand = Commands.getStartConfigServerCommand( config.getCfgSrvPort(),
                Sets.newHashSet( node ) );
        }
        else if ( nodeType == NodeType.DATA_NODE )
        {
            startNodeCommand = Commands
                .getStartDataNodeCommand( config.getDataNodePort(), Sets.newHashSet( node ) );
        }
        else
        {
