...
public class Commands extends CommandsSingleton
{
    ...
    public static List<Command> getInstallationCommands( Config config )
    {
        ...
        Command startConfigServersCommand = getStartConfigServerCommand( config.getCfgSrvPort(),
    ...
    public static Command getAddIpHostToEtcHostsCommand( String domainName, Set<Agent> agents )
    {
        ...
        for ( Agent agent : agents )
        {
            ...
            requestBuilders.add(
                ( AgentRequestBuilder ) new AgentRequestBuilder( agent, appendHosts.toString() ).withTimeout( 90 ) );
    ...
    public static List<Command> getAddRouterCommands( Config config, Agent newRouterAgent )
    {
        ...
        commands.add( getInstallCommand( Sets.newHashSet( newRouterAgent ) ) );

        commands.add( getStopNodeCommand( Sets.newHashSet( newRouterAgent ) ) );
    ...
    public static List<Command> getAddDataNodeCommands( Config config, Agent newDataNodeAgent )
    {
        ...
        commands.add( getInstallCommand( Sets.newHashSet( newDataNodeAgent ) ) );

        commands.add( getStopNodeCommand( Sets.newHashSet( newDataNodeAgent ) ) );

        commands.add( getAddIpHostToEtcHostsCommand( config.getDomainName(), clusterMembers ) );

        commands
            .add( getSetReplicaSetNameCommand( config.getReplicaSetName(), Sets.newHashSet( newDataNodeAgent ) ) );
        ...
        Command findPrimaryNodeCommand = getFindPrimaryNodeCommand( config.getDataNodes().iterator().next(),
