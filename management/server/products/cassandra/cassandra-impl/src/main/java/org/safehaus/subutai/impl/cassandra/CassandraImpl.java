    ...
    public UUID installCluster( final Config config ) {
        ...
        executor.execute( new Runnable() {
            ...
            public void run() {
                ...
                try {
                    po.addLog( String.format( "Creating %d lxc containers for Cassandra cluster...",
                    ...
                    if ( dbManager.saveInfo( Config.PRODUCT_KEY, config.getClusterName(), config ) ) {
                        ...
                        if ( networkManager.configHostsOnAgents( new ArrayList<Agent>( config.getNodes() ),
                                config.getDomainName() ) && networkManager
                                .configSshOnAgents( new ArrayList<Agent>( config.getNodes() ) ) ) {
                        ...
                        Command setClusterNameCommand = Commands.getConfigureCommand( config.getNodes(),
                        ...
                        Command setDataDirCommand = Commands.getConfigureCommand( config.getNodes(),
                        ...
                        if ( setDataDirCommand.hasSucceeded() ) {
                            ...
                            po.addLogFailed(
                                    String.format( "Installation failed, %s", setDataDirCommand.getAllErrors() ) );
                            return;
                        }

                        // setting commit log directory
                        po.addLog( "Setting commit directory: " + config.getCommitLogDirectory() );
                        Command setCommitDirCommand = Commands.getConfigureCommand( config.getNodes(),
                        ...
                        if ( setCommitDirCommand.hasSucceeded() ) {
                            ...
                            po.addLogFailed(
                                    String.format( "Installation failed, %s", setCommitDirCommand.getAllErrors() ) );
                            return;
                        }
                        ...
                        if ( setSavedCacheDirCommand.hasSucceeded() ) {
                            ...
                            po.addLogFailed( String.format( "Installation failed, %s",
                                    setSavedCacheDirCommand.getAllErrors() ) );
                            return;
                        }
                        ...
                        if ( setRpcAddressCommand.hasSucceeded() ) {
                            ...
                            po.addLogFailed(
                                    String.format( "Installation failed, %s", setRpcAddressCommand.getAllErrors() ) );
                            return;
                        }
                        ...
                        Command setListenAddressCommand =
                                Commands.getConfigureRpcAndListenAddressesCommand( config.getNodes(),
                                        "listen_address" );
                        ...
                        if ( setListenAddressCommand.hasSucceeded() ) {
                            ...
                            po.addLogFailed( String.format( "Installation failed, %s",
                                    setListenAddressCommand.getAllErrors() ) );
                            return;
                        }
                        ...
                        Command setSeedsCommand =
                                Commands.getConfigureCommand( config.getNodes(), "seeds " + sb.toString() );
                        ...
                        if ( setSeedsCommand.hasSucceeded() ) {
                            ...
                            po.addLogFailed(
                                    String.format( "Installation failed, %s", setSeedsCommand.getAllErrors() ) );
                            return;
                        }
