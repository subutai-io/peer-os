 ...
 */
package org.safehaus.subutai.ui.pig.manager;
...
public class Manager {
	...
	public Manager() {
			...
			public void buttonClick(Button.ClickEvent clickEvent) {
				if (config != null) {
					...
					alert.getOk().addClickListener(new Button.ClickListener() {
						@Override
						public void buttonClick(Button.ClickEvent clickEvent) {
							UUID trackID = PigUI.getPigManager().uninstallCluster(config.getClusterName());
		...
		addNodeBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(Button.ClickEvent clickEvent) {
				if (config != null) {
					org.safehaus.subutai.api.hadoop.Config info = PigUI.getHadoopManager().getCluster(config.getClusterName());
					...
					if (info != null) {
						...
						if (!nodes.isEmpty()) {
							...
							show("All nodes in corresponding Hadoop cluster have Pig installed");
						}
					} else {
			...
			destroyBtn.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(Button.ClickEvent clickEvent) {
					...
					alert.getOk().addClickListener(new Button.ClickListener() {
						@Override
						public void buttonClick(Button.ClickEvent clickEvent) {
							UUID trackID = PigUI.getPigManager().destroyNode(config.getClusterName(), agent.getHostname());
							ProgressWindow window = new ProgressWindow(PigUI.getExecutor(), PigUI.getTracker(), trackID, Config.PRODUCT_KEY);
							window.getWindow().addCloseListener(new Window.CloseListener() {
								@Override
								public void windowClose(Window.CloseEvent closeEvent) {
		...
		table.addItemClickListener(new ItemClickEvent.ItemClickListener() {
			...
			public void itemClick(ItemClickEvent event) {
				if (event.isDoubleClick()) {
					...
					if (lxcAgent != null) {
						TerminalWindow terminal = new TerminalWindow(Util.wrapAgentToSet(lxcAgent), PigUI.getExecutor(), PigUI.getCommandRunner(), PigUI.getAgentManager());
