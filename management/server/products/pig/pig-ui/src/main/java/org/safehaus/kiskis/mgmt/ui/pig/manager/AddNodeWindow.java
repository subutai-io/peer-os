    ...
    public AddNodeWindow(final Config config, Set<Agent> nodes) {
        ...
        addNodeBtn.addListener(new Button.ClickListener() {
            ...
            public void buttonClick(Button.ClickEvent event) {
                ...
                final UUID trackID = PigUI.getPigManager().addNode(config.getClusterName(), agent.getHostname());
                PigUI.getExecutor().execute(new Runnable() {

                    public void run() {
                        while (track) {
                            ProductOperationView po = PigUI.getTracker().getProductOperation(Config.PRODUCT_KEY, trackID);
                            if (po != null) {
