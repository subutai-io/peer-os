...
public class AddNodeWindow extends Window
{
    ...
    public AddNodeWindow( final Nutch nutch, final Tracker tracker, final ExecutorService executorService,
                          final NutchConfig config, Set<Agent> nodes )
    {
        super( "Add New Node" );
        setModal( true );
        ...
        addNodeBtn.addClickListener( new Button.ClickListener()
        {
            ...
            public void buttonClick( Button.ClickEvent clickEvent )
            {
                ...
                final UUID trackID = nutch.addNode( config.getClusterName(), agent.getHostname() );
                ok.setEnabled( false );
                executorService.execute( new Runnable()
                {
                    ...
                    public void run()
                    {
                        while ( track )
                        {
                            ProductOperationView po = tracker.getProductOperation( NutchConfig.PRODUCT_KEY, trackID );
                            if ( po != null )
                            {
                                setOutput(
                                        po.getDescription() + "\nState: " + po.getState() + "\nLogs:\n" + po.getLog() );
