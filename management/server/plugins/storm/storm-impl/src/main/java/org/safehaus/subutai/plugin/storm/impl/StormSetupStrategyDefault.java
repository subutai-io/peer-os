...
public class StormSetupStrategyDefault implements ClusterSetupStrategy {
    ...
    public StormSetupStrategyDefault(StormImpl manager, StormConfig config, Environment environment, ProductOperation po) {
    ...
    public ConfigBase setup() throws ClusterSetupException {
        ...
        for(Node n : environment.getNodes()) {
            if(!n.getTemplate().getProducts().contains(Commands.PACKAGE_NAME))
                throw new ClusterSetupException(String.format(
                        "Node %s does not have Storm installed",
                        n.getAgent().getHostname()));
                ...
                if(n.getNodeGroupName().equals(StormService.NIMBUS.toString()))
                    if(!n.getTemplate().getProducts().contains(zk_pack))
                        throw new ClusterSetupException(
                                "Nimbus node does not have Zookeeper installed");
            }
        }
        ...
        if(config.isExternalZookeeper()) {
            ...
            if(zk == null)
                throw new ClusterSetupException("Zookeeper cluster not found: "
    ...
    public void configure() throws ClusterSetupException {
        ...
        for(Map.Entry<String, String> e : paramValues.entrySet()) {
            ...
            if(!cmd.hasSucceeded())
                throw new ClusterSetupException("Failed to configure: "
                        + cmd.getAllErrors());
        }
    }
