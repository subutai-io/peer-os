...
public class StatusHandler extends AbstractHandler
{
    ...
    public StatusHandler( StormImpl manager, String clusterName, String hostname )
    {
        ...
        this.trackerOperation =
                manager.getTracker().createTrackerOperation( StormConfig.PRODUCT_NAME, "Check status of " + hostname );
    }
    ...
    public void run()
    {
        ...
        for ( StormService service : services )
        {
            ...
            Command cmd = manager.getCommandRunner().createCommand( new RequestBuilder( s ), set );
            manager.getCommandRunner().runCommand( cmd );
            result = result && cmd.hasSucceeded();

            po.addLog( String.format( "Storm %s is %s running on %s", service, cmd.hasSucceeded() ? "" : "not",
                    agent.getHostname() ) );
        }
        if ( result )
        {
