...
public class StopHandler extends AbstractHandler
{
    ...
    public StopHandler( StormImpl manager, String clusterName, String hostname )
    {
        ...
        this.productOperation =
                manager.getTracker().createProductOperation( StormConfig.PRODUCT_NAME, "Stop node " + hostname );
    }
    ...
    public void run()
    {
        ...
        for ( StormService service : services )
        {
            ...
            po.addLog( String.format( "Storm %s %s stopped on %s", service, cmd.hasSucceeded() ? "" : "not",
                    agent.getHostname() ) );
        }
        if ( result )
        {
