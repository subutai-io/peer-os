...
public class StartHandler extends AbstractHandler
{
    ...
    public StartHandler( StormImpl manager, String clusterName, String hostname )
    {
        ...
        this.productOperation =
                manager.getTracker().createProductOperation( StormConfig.PRODUCT_NAME, "Start node " + hostname );
    }
    ...
    public void run()
    {
        ...
        for ( StormService service : services )
        {
            ...
            po.addLog( String.format( "Storm %s %s started on %s", service, cmd.hasSucceeded() ? "" : "not",
                    agent.getHostname() ) );
        }
        if ( result )
        {
