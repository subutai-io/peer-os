...
public class Manager {
                ...
                if(event.isDoubleClick()) {
                    ...
                    if(lxcAgent != null) {
                        TerminalWindow terminal = new TerminalWindow(Sets.newHashSet(lxcAgent),
                                StormUI.getExecutor(), StormUI.getCommandRunner(), StormUI.getAgentManager());
    ...
    public void refreshClustersInfo() {
        ...
        List<StormConfig> clustersInfo = StormUI.getManager().getClusters();
    ...
    private void destroyClusterHandler() {
        ...
        ProgressWindow window = new ProgressWindow(StormUI.getExecutor(), StormUI.getTracker(), trackID,
                        ...
                        alert.getOk().addClickListener(new Button.ClickListener() {
                            ...
                            public void buttonClick(Button.ClickEvent clickEvent) {
                                UUID trackID = StormUI.getManager().destroyNode(
                                        ...
                                        agent.getHostname());
                                ProgressWindow window = new ProgressWindow(StormUI.getExecutor(), StormUI.getTracker(),
                                        trackID, StormConfig.PRODUCT_NAME);
                                window.getWindow().addCloseListener(new Window.CloseListener() {
                                    @Override
                                    public void windowClose(Window.CloseEvent closeEvent) {
                                        refreshClustersInfo();
                        ...
                        public void run() {
                            ...
                            while(po == null || po.getState() == ProductOperationState.RUNNING) {
                                po = StormUI.getTracker().getProductOperation(
                                        StormConfig.PRODUCT_NAME, trackId);
                            }
