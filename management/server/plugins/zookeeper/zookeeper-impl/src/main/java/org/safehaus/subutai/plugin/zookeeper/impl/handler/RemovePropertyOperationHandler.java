	...
	public void run() {
		...
		if (removePropertyCommand.hasSucceeded()) {
			...
			if (count.get() == config.getNodes().size()) {
				...
				productOperation.addLogFailed( String.format( "Failed to restart cluster, %s", restartCommand.getAllErrors() ) );
			}
		} else {
			productOperation.addLogFailed( String.format( "Removing property failed, %s", removePropertyCommand.getAllErrors() ) );
		}
	}
}
