...
public class Manager
{
    ...
    public Manager( final ExecutorService executorService, ServiceLocator serviceLocator ) throws NamingException
    {
        ...
        clusterCombo.addValueChangeListener( new Property.ValueChangeListener()
        {
            ...
            {
                config = ( JettyConfig ) event.getProperty().getValue();
                refreshUI();
                checkAllNodes();
            }
        } );
    ...
    {
        List<JettyConfig> info = jetty.getClusters();
        JettyConfig clusterInfo = ( JettyConfig ) clusterCombo.getValue();
        clusterCombo.removeAllItems();
        if ( info != null && !info.isEmpty() )
        {
            for ( JettyConfig JettyInfo : info )
            {
                clusterCombo.addItem( JettyInfo );
                clusterCombo.setItemCaption( JettyInfo, JettyInfo.getClusterName() );
            }
            if ( clusterInfo != null )
            {
                for ( JettyConfig JettyInfo : info )
                {
                    if ( JettyInfo.getClusterName().equals( clusterInfo.getClusterName() ) )
                    {
                        clusterCombo.setValue( JettyInfo );
                        return;
                    }
                }
            }
