...
public class CassandraImpl implements Cassandra {
    ...
    public UUID installCluster(final CassandraConfig config) {
        final ProductOperation po = tracker.createProductOperation(CassandraConfig.PRODUCT_KEY, "Installing Cassandra");

        executor.execute(new Runnable() {

            public void run() {
                if (dbManager.getInfo(CassandraConfig.PRODUCT_KEY, config.getClusterName(), CassandraConfig.class) != null) {
                ...
                try {
                    ...
                    if (dbManager.saveInfo(CassandraConfig.PRODUCT_KEY, config.getClusterName(), config)) {
...
//                        sb.append('"');
    ...
    public UUID uninstallCluster(final String clusterName) {
        final ProductOperation po
                = tracker.createProductOperation(CassandraConfig.PRODUCT_KEY,
                String.format("Destroying cluster %s", clusterName));

        executor.execute(new Runnable() {

            public void run() {
                CassandraConfig config = dbManager.getInfo(CassandraConfig.PRODUCT_KEY, clusterName, CassandraConfig.class);
                if (config == null) {
                ...
                po.addLog("Updating db...");
                if (dbManager.deleteInfo(CassandraConfig.PRODUCT_KEY, config.getClusterName())) {
...
//                    po.addLogFailed(String.format("Status check failed, %s", statusServiceCommand.getAllErrors()));
    ...
    public ClusterSetupStrategy getClusterSetupStrategy( final Environment environment, final CassandraConfig config,
                                                         final ProductOperation po ) {
        return new CassandraSetupStrategy();
    }
