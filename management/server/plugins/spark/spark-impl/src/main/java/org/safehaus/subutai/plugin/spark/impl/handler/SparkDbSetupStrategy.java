...
public class SparkDbSetupStrategy implements ClusterSetupStrategy {
    ...
    public SparkDbSetupStrategy(final ProductOperation po,
                                final Spark sparkManager,
    ...
    public SparkDbSetupStrategy(Environment environment, ProductOperation po, Spark sparkManager, SparkClusterConfig sparkClusterConfig) {
    ...
    private void check() throws ClusterSetupException {
        ...
        if (!checkInstalledCommand.hasCompleted()) {
            throw new ClusterSetupException("Failed to check presence of installed ksks packages\nInstallation aborted");
        }
    ...
    private void install() throws ClusterSetupException {
        ...
        //save to db
        try {
            SparkImpl.getPluginDAO().saveInfo(SparkClusterConfig.PRODUCT_KEY, sparkClusterConfig.getClusterName(), sparkClusterConfig);
            ...
            if (installCommand.hasSucceeded()) {
                ...
                Command setMasterIPCommand = Commands
                        .getSetMasterIPCommand(sparkClusterConfig.getMasterNode(), sparkClusterConfig.getAllNodes());
                ...
                if (setMasterIPCommand.hasSucceeded()) {
                    ...
                    Command addSlavesCommand = Commands
                            .getAddSlavesCommand(sparkClusterConfig.getSlaveNodes(), sparkClusterConfig.getMasterNode());
                    ...
                    if (addSlavesCommand.hasSucceeded()) {
                        ...
                        if (okCount.get() >= sparkClusterConfig.getAllNodes().size()) {
                            ...
                            throw new ClusterSetupException(String.format("Failed to start cluster, %s", startNodesCommand.getAllErrors()));
                        }

                    } else {
                        throw new ClusterSetupException(String.format("Failed to register slaves with master, %s", addSlavesCommand.getAllErrors()));
                    }
                } else {
                    throw new ClusterSetupException(String.format("Setting master IP failed, %s", setMasterIPCommand.getAllErrors()));
                }
            ...
            throw new ClusterSetupException("Could not save cluster info to DB! Please see logs\nInstallation aborted");
        }
    }
}
