...
public class HadoopDbSetupStrategy implements ClusterSetupStrategy {
			...
			if (hadoopManager.getCluster( hadoopClusterConfig.getClusterName()) != null) {
				...
				try {
					...
					Set<Agent> cfgServers = containerManager
							.clone( HadoopClusterConfig.TEMPLATE_NAME, 3, HadoopImpl.getAgentManager().getPhysicalAgents(),
									getNodePlacementStrategyByNodeType(NodeType.MASTER_NODE));

					po.addLog(String.format("Creating %d slave nodes...", hadoopClusterConfig.getCountOfSlaveNodes()));
					Set<Agent> dataNodes = containerManager
							.clone( HadoopClusterConfig.TEMPLATE_NAME, hadoopClusterConfig.getCountOfSlaveNodes(), HadoopImpl.getAgentManager().getPhysicalAgents(),
	...
	private void installHadoopCluster() throws ClusterSetupException {
		...
		if (HadoopImpl.getNetworkManager().configHostsOnAgents( hadoopClusterConfig.getAllNodes(), hadoopClusterConfig.getDomainName()) &&
