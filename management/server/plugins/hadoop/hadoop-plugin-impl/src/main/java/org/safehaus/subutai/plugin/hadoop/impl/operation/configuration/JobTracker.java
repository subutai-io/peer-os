...
public class JobTracker {
    ...
    public JobTracker( final HadoopImpl parent, HadoopClusterConfig hadoopClusterConfig ) {
    ...
    public UUID start() {
        ...
        parent.getExecutor().execute( new Runnable() {
            ...
            public void run() {
                ...
                final Agent node = parent.getAgentManager()
                                         .getAgentByHostname( hadoopClusterConfig.getJobTracker().getHostname() );
                if ( node == null ) {
        ...
        parent.getExecutor().execute( new Runnable() {
            ...
            public void run() {
                ...
                if ( NodeState.UNKNOWN.equals( nodeState ) ) {
                    po.addLogFailed(
                            String.format( "Failed to check status of %s, %s", hadoopClusterConfig.getClusterName(),
                                    hadoopClusterConfig.getJobTracker().getHostname() ) );
                }
                else {
                    po.addLogDone(
                            String.format( "JobTracker of %s is %s", hadoopClusterConfig.getJobTracker().getHostname(),
                                    nodeState ) );
                }
            }
        } );
