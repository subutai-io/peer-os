...
public class HadoopSetupStrategyTest
{
    ...
    public void testSetup() throws ClusterSetupException
    {
        ...
        when( iterator.next() ).thenReturn( containerHost ).thenReturn( containerHost2 ).thenReturn( containerHost3 )
                               .thenReturn( containerHost4 ).thenReturn( containerHost ).thenReturn( containerHost2 )
                               .thenReturn( containerHost3 ).thenReturn( containerHost4 );
        ...
        when( agent.getUuid() ).thenReturn( UUID.randomUUID() ).thenReturn( UUID.randomUUID() )
                               .thenReturn( UUID.randomUUID() ).thenReturn( UUID.randomUUID() )
                               .thenReturn( UUID.randomUUID() ).thenReturn( UUID.randomUUID() )
                               .thenReturn( UUID.randomUUID() ).thenReturn( UUID.randomUUID() );
    ...
    public void testSetMasterNodes() throws ClusterSetupException
    {
        ...
        when( iterator.hasNext() ).thenReturn( true ).thenReturn( true ).thenReturn( true ).thenReturn( true )
                                  .thenReturn( false );
        when( iterator.next() ).thenReturn( containerHost ).thenReturn( containerHost2 ).thenReturn( containerHost3 )
                               .thenReturn( containerHost4 );
        when( mySet.size() ).thenReturn( 5 );
        when( containerHost.getAgent() ).thenReturn( agent );
        when( containerHost2.getAgent() ).thenReturn( agent );
        when( containerHost3.getAgent() ).thenReturn( agent );
        when( containerHost4.getAgent() ).thenReturn( agent );
        when( agent.getUuid() ).thenReturn( uuid ).thenReturn( uuid2 ).thenReturn( uuid3 ).thenReturn( uuid4 )
                               .thenReturn( UUID.randomUUID() ).thenReturn( UUID.randomUUID() )
                               .thenReturn( UUID.randomUUID() ).thenReturn( UUID.randomUUID() );
