...
public class PluginDAO
{
    ...
    public <T> List<T> getInfo( String source, Class<T> clazz )
    {
        ...
        try
        {
            ...
            try
            {
                rs = dbManager.executeQuery2( "select info from product_info where source = ?", source.toLowerCase() );
            }
            catch ( DBException e )
            {
                LOG.severe( e.getMessage() );
    ...
    public <T> T getInfo( String source, String key, Class<T> clazz )
    {
        ...
        try
        {
            ...
            try
            {
                rs = dbManager.executeQuery2( "select info from product_info where source = ? and key = ?",
