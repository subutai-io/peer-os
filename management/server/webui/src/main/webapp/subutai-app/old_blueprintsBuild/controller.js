...
function BlueprintsBuildFormCtrl($scope, environmentService, SweetAlert, ngDialog, $stateParams) {
	...
	function removeNode(peer, strategies, itemKey, originalKey) {
		vm.deleteNode.push(originalKey);
		vm.groupList[peer][strategies].splice(itemKey, 1);
	}

	function removeGroup(peer) {
		for(var i = 0; i < vm.groupList[peer].length; i++) {
			for(var j = 0; j < vm.groupList[peer][i].length; j++) {
				vm.deleteNode.push(vm.groupList[peer][i][j].originalKey);
			}
		}
		console.log(vm.deleteNode);
		delete vm.groupList[peer];
	}
