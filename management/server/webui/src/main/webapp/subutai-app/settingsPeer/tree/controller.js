...
function SettingsPeerCtrl ($scope, SettingsPeerSrv, ngDialog) {
	...
	// TODO: get trust relation and show on diagram

/*	var relations = {};
	function getRelations() {
		SettingsPeerSrv.getRelations().success (function (data)) {
			relations = data;
			console.log (data);
		}
	}
	...
	function traverse (tree) {
		...
		for (var i = 0; i < tree.length; ++i) {
			...
			span.innerHTML = " " + tree[i].name;
			$(span).css ({"font-weight": "400", "font-size": "14px"});
			span.className = "node";
			var label = document.createElement ("i");
			label.className = "fa";
			if (tree[i].children === undefined || tree[i].children.length === 0) {
				label.className += " fa-server";
				label.style.color = "#945309";
			}
			else {
				label.className += " fa-minus-circle";
				label.style.color = "#9C0606";
			}
			$(span).prepend (label);
...
/*	var margin = {top: 20, right: 120, bottom: 20, left: 120},
	...
	var svg = d3.select("#peer_tree").append("svg")
