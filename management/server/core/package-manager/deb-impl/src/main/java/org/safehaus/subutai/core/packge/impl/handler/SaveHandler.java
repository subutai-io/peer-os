...
public class SaveHandler extends AbstractHandler<Collection<PackageInfo>>
{
    ...
    public Collection<PackageInfo> performAction()
    {
        ...
        {
            RequestBuilder rb = new RequestBuilder( "dpkg -l > " + packageManager.getFilename() )
    ...
    private boolean checkTargetLocation( Agent a )
    {
        ...
        else
        {
            ...
            if ( !equalsZero( res.getExitCode() ) )
            {
                getLogger().error( "Target location can not be created: " + res.getStdErr() );
                return false;
            }
        }
        return true;
    ...
    {
        Command c = packageManager.getCommandRunner()
                                  .createCommand( new RequestBuilder( cmd ), new HashSet<>( Arrays.asList( a ) ) );
