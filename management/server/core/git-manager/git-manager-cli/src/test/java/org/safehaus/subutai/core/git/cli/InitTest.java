...
public class InitTest
{
    ...
    {
        new Init( null, mock( AgentManager.class ) );
    }
    ...
    {
        new Init( mock( GitManager.class ), null );
    }
    ...
    {
        Init init = new Init( mock( GitManager.class ), mock( AgentManager.class ) );
    ...
    {
        Init init = new Init( gitManager, agentManager );
        init.setHostname( HOSTNAME );

        init.doExecute();

        verify( gitManager ).init( eq( agent ), anyString() );
    }
    ...
    {
        Mockito.doThrow( new GitException( ERR_MSG ) ).when( gitManager ).init( eq( agent ), anyString() );
        Init init = new Init( gitManager, agentManager );
        init.setHostname( HOSTNAME );
