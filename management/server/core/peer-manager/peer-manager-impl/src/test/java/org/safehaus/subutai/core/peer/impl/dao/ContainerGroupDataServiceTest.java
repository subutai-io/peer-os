...
public class ContainerGroupDataServiceTest
{
    ...
    @Mock
    TypedQuery<ContainerGroupEntity> typedQuery;
    @Mock
    ContainerGroupEntity item;

    ContainerGroupDataService service;


    @Before
    public void setUp() throws Exception
    {
        service = new ContainerGroupDataService( entityManagerFactory );
        when( entityManagerFactory.createEntityManager() ).thenReturn( em );
        when( em.getTransaction() ).thenReturn( transaction );
        when( transaction.isActive() ).thenReturn( true );
        when( em.createQuery( anyString(), eq( ContainerGroupEntity.class ) ) ).thenReturn( typedQuery );
    }
