...
/**
 * Servlet that either throws and exception if errorException request parameter is set and is a correct FQN of Throwable
 * subtype. or sends an error if errorCode request parameter is provided.
 *
 * Used to test error page registration.
 *
 * errorCode takes precedence or errorException.
 *
 * If neither is provided or errorCode is not valid http error code or errorException is not correct will throw an
 * IllegalArgumentException. errorCode must in in 400 or 500 range to be considered valid.
 */
