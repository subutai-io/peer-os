...
public class UsProvisionalApplication implements Converter{
	...
	public UsProvisionalApplication(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				else if (childElement.getName().equals("us-provisional-application-status"))
				{
					usProvisionalApplicationStatus = childElement.getValue();
				}
				else
				{
