...
public class Applicants extends SingleCollection<Converter>{

	private static final String title = "Applicants";
	
	protected Logger logger;
	
	public Applicants(Logger logger) {
		super();
		this.logger = logger;
	}
	
	public Applicants(Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (childElement.getNodeName().equals("applicant")) {
					elements.add(new Applicant(childElement, logger));
				}
				else
				{
	...
	public Applicants(org.jdom2.Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("applicant")) {
					elements.add(new Applicant(childElement, logger));
				}
				else
				{
