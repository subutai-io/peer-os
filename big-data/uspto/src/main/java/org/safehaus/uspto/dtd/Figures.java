...
public class Figures implements Converter{
	...
	public Figures(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("number-of-drawing-sheets")) {
					numberOfDrawingSheets = childElement.getValue();
				}
				else if (childElement.getName().equals("number-of-figures")) {
					numberOfFigures = childElement.getValue();
				}
				else
				{
