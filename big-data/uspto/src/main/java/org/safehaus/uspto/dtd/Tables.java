...
public class Tables implements Converter{

	private static final String title = "Tables";
	...
	public Tables(Logger logger) {
		this.logger = logger;
	}
	
	public Tables(Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				else if (childElement.getNodeName().equals("table")) {
					//TODO complete this.
					//logger.warn("table: {}", childElement.getTextContent());
				}
				else
				{
	...
	public Tables(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				else if (childElement.getName().equals("table")) {
					//TODO complete this.
					//logger.warn("table: {}", childElement.getTextContent());
				}
				else
