...
public class UsMath implements Converter{
	...
	public UsMath(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < attributes.size(); i++)
		{
			...
			if (attribute.getName().equals("idrefs")) {
				idrefs = attribute.getValue();
			}
			else if (attribute.getName().equals("nb-file")) {
				nbFile = attribute.getValue();
			}
			else
			{
