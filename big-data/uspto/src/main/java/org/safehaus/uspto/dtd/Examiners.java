...
public class Examiners implements Converter{
	...
	public Examiners(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("primary-examiner")) {
					primaryExaminer  = new Examiner(childElement, logger);
				}
				else if (childElement.getName().equals("assistant-examiner")) {
					assistantExaminer  = new Examiner(childElement, logger);
				}
				else if (childElement.getName().equals("authorized-officer")) {
					authorizedOfficer  = new Examiner(childElement, logger);
				}
				else
				{
