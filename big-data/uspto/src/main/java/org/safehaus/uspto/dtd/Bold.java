...
public class Bold extends SingleCollection<Converter>{

	private static final String title = "Bold";
	
	protected Logger logger;

	public Bold(Logger logger) {
		super();
		this.logger = logger;
	}
	
	public Bold(Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
	...
	public Bold(org.jdom2.Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("i")) {
					elements.add(new Italic(childElement, logger));
				}
