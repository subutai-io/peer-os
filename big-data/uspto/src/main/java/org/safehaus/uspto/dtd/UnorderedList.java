...
public class UnorderedList extends SingleCollection<ListItem>{

private static final String title = "UnorderedList";
	...
	public UnorderedList(Logger logger) {
		super();
		this.logger = logger;
	}
	
	public UnorderedList(Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i=0; i < nodeMap.getLength(); i++)
		{
			...
			if (node.getNodeType() == Node.ATTRIBUTE_NODE) {
				...
				else if (attribute.getNodeName().equals("list-style")) {
	...
	public UnorderedList(org.jdom2.Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i=0; i < attributes.size(); i++)
		{
			...
			else if (attribute.getName().equals("list-style")) {
				listStyle = attribute.getValue();
			}
			else if (attribute.getName().equals("compact")) {
				compact = attribute.getValue();
			}
			else
			{
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("li")) {
					elements.add(new ListItem(childElement, logger));
				}
				else
				{
