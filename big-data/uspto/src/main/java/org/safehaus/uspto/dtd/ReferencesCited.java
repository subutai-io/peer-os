...
public class ReferencesCited extends SingleCollection<Citation>{

	private static final String title = "ReferencesCited";
	
	protected Logger logger;
	
	public ReferencesCited(Logger logger) {
		super();
		this.logger = logger;
	}
	
	public ReferencesCited(Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (childElement.getNodeName().equals("citation")) {
					elements.add(new Citation(childElement, logger));
				}
				else
				{
	...
	public ReferencesCited(org.jdom2.Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("citation")) {
					elements.add(new Citation(childElement, logger));
				}
				else
				{
