...
public class ClassificationNational implements Converter{
	...
	public ClassificationNational(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("country")) {
					...
					furtherClassification = childElement.getValue();
				}
				else if (childElement.getName().equals("additional-info")) {
					additionalInfo = childElement.getValue();
				}
				else
				{
