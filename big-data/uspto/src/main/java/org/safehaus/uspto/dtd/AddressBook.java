...
public class AddressBook implements Converter{

	private static final String title = "AddressBook";
	...
	public AddressBook(Logger logger) {
		this.logger = logger;
	}
	
	public AddressBook(Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				else if (childElement.getNodeName().equals("address")) {
					address = new Address(childElement, logger);
				}
				else
				{
	...
	public AddressBook(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
	...
	public JSONObject toJSon() {
		...
		if (address != null)
		{
			jsonObject.put(address.getTitle(), address.toJSon());
		}
		return jsonObject;
	}
	...
	public BasicDBObject toBasicDBObject() {
		...
		if (address != null)
		{
			basicDBObject.put(address.getTitle(), address.toBasicDBObject());
		}
