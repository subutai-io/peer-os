...
public class ClassificationIpcr implements Converter{
	...
	public ClassificationIpcr(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("ipc-version-indicator")) {
					ipcVersionIndicator = new IpcVersionIndicator(childElement, logger);
				}
				else if (childElement.getName().equals("classification-level")) {
					classificationLevel = childElement.getValue();
				}
