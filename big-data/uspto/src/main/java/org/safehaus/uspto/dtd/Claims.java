...
public class Claims extends SingleCollection<Claim>{

	protected Logger logger;
	
	private static final String title = "Claims";
	...
	public Claims(Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (childElement.getNodeName().equals("claim")) {
					elements.add(new Claim(childElement, logger));
				}
				else
				{
	...
	public Claims(org.jdom2.Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("claim")) {
					elements.add(new Claim(childElement, logger));
				}
				else
				{
