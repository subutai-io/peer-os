...
public class ClassificationCpcText implements Converter{

	private static final String title = "ClassificationCpcText";
	...
	public ClassificationCpcText(Logger logger) {
		this.logger = logger;
	}
	
	public ClassificationCpcText(Element element, Logger logger)
	{
		this.logger = logger;
	...
	public ClassificationCpcText(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Text) {
				...
				this.text = text.getTextNormalize();
			}
