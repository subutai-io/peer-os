...
public class PctRegionalPublishingData implements Converter{

	private static final String title = "PctRegionalPublishingData";
	
	protected Logger logger;
	
	private DocumentId documentId;
	private GazetteReference gazetteReference;
	
	public PctRegionalPublishingData(Logger logger) {
		this.logger = logger;
	}

	public PctRegionalPublishingData(Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int j=0; j < nodes.getLength(); j++)
		{
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				else if (childElement.getNodeName().equals("gazette-reference"))
				{
					gazetteReference = new GazetteReference(childElement, logger);
				}
				else
				{
	...
	public PctRegionalPublishingData(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				else if (childElement.getName().equals("gazette-reference"))
				{
					gazetteReference = new GazetteReference(childElement, logger);
				}
				else
	...
	public String toString() {
		...
		{
			toStringBuffer.append(" ");
			toStringBuffer.append(gazetteReference);
		}
	...
	public JSONObject toJSon() {
		...
		if (gazetteReference != null)
		{
			jsonObject.put(gazetteReference.getTitle(), gazetteReference.toJSon());
		}
		return jsonObject;
	}
	...
	public BasicDBObject toBasicDBObject() {
		...
		if (gazetteReference != null)
		{
			basicDBObject.put(gazetteReference.getTitle(), gazetteReference.toBasicDBObject());
		}
