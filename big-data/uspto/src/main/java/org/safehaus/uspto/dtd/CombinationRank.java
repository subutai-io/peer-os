...
public class CombinationRank implements Converter{

	private static final String title = "CombinationRank";
	...
	public CombinationRank(Logger logger) {
		this.logger = logger;
	}
	
	public CombinationRank(Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (childElement.getNodeName().equals("rank-number")) {
					rankNumber = childElement.getTextContent();
				}
				else if (childElement.getNodeName().equals("classification-cpc")) {
	...
	public CombinationRank(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("rank-number")) {
					rankNumber = childElement.getValue();
				}
				else if (childElement.getName().equals("classification-cpc")) {
					classificationCpc = new ClassificationCpc(childElement, logger);
				}
				else
				{
	...
	public String toString() {
		...
		if (rankNumber != null)
		{
			toStringBuffer.append(" RankNumber: ");
			toStringBuffer.append(rankNumber);
		}
	...
	public JSONObject toJSon() {
		...
		if (rankNumber != null)
		{
			jsonObject.put("RankNumber", rankNumber);
		}
		if (classificationCpc != null)
		{
	...
	public BasicDBObject toBasicDBObject() {
		...
		if (rankNumber != null)
		{
			basicDBObject.put("RankNumber", rankNumber);
		}
		if (classificationCpc != null)
		{
