...
public class GrantTerms implements Converter{
	...
	public GrantTerms(org.jdom2.Element element, Logger logger)
	{
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("length-of-grant")) {
					grantLength = childElement.getValue();
				}
				else if (childElement.getName().equals("us-term-extension")) {
					usTermExtension = childElement.getValue();
				}
				else if (childElement.getName().equals("disclaimer")) {
					disclaimer = new Disclaimer(childElement, logger);
				}
				else
				{
