...
public class Assignees extends SingleCollection<Converter>{

	private static final String title = "Assignees";
	
	protected Logger logger;
	
	public Assignees(Logger logger) {
		super();
		this.logger = logger;
	}
	
	public Assignees(Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (childElement.getNodeName().equals("assignee")) {
					elements.add(new Assignee(childElement, logger));
				}
				else
				{
	...
	public Assignees(org.jdom2.Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("assignee")) {
					elements.add(new Assignee(childElement, logger));
				}
				else
				{
