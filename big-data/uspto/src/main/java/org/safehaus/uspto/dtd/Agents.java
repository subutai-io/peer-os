...
public class Agents extends SingleCollection<Agent>{

	private static final String title = "Agents";
	
	protected Logger logger;
	
	public Agents(Logger logger) {
		super();
		this.logger = logger;
	}
	
	public Agents(Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				...
				if (childElement.getNodeName().equals("agent")) {
					elements.add(new Agent(childElement, logger));
				}
				else
				{
	...
	public Agents(org.jdom2.Element element, Logger logger)
	{
		super(element);
		this.logger = logger;
		...
		for (int i=0; i < nodes.size(); i++)
		{
			...
			if (node.getCType() == Content.CType.Element) {
				...
				if (childElement.getName().equals("agent")) {
					elements.add(new Agent(childElement, logger));
				}
				else
				{
